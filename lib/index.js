  ...
  fetch(BALANCE_URL, { cache: 'no-store', signal: ctrl ? ctrl.signal : undefined })
    ...
    .then(function (data) {
      if (data && data.ok) {
        ...
        state.balance = nb
        state.currency = nc
        state.provider = data.provider || 'DeepSeek'
        state.balanceKind = data.balanceKind || 'balance'
        state.message = ''
    ...
    fetch(LAST_TURN_URL, { cache: 'no-store' })
      ...
      .then(function (d) {
        ...
        if (d.seq > lastCostSeq) {
          ...
          // 每轮结束同步刷新：大会话数字 + 今日累计
          if (typeof d.sessionTokens === 'number') {
            state.balance = d.sessionTokens
            state.currency = 'tokens'
            if (d.todayTokens !== null && d.todayTokens !== undefined) state.todayUsage = d.todayTokens
            render()
          }
        }
      })
...
function apply(ctx) {
    ...
    let gifBytes = null
    // 每轮对话消耗统计：按「根会话」聚合——主会话与其派生的子代理（spawn/fork，
    // header.parentSession 存在）全部归并进当前对话，避免子代理并行时互相顶掉数字。
    // turn 桶跟随根会话自身的 turn/start / turn/end 开启与结算。
    let turnAggs = new Map() // rootId -> { turn, cost, tokens, lastTs } 根会话进行中的一轮
    let sessionTotals = new Map() // rootId -> { tokens, calls, lastTs } 当前对话累计（含子代理）
    let sessionRoots = new Map() // sessionId -> rootId 派生会话到根会话的归并表
    let currentRootId = null
    let lastTurn = null // { turn, amount, tokens, ts }
    let lastTurnSeq = 0
    const disposers = []

    // 子代理 / fork 会话的 header 带 parentSession；沿链归并到根会话
    function rootOf(session) {
      const sid = session && session.id ? session.id : 'default'
      if (sessionRoots.has(sid)) return sessionRoots.get(sid)
      let root = sid
      try {
        const header = session && session.header
        const parent = header && typeof header.parentSession === 'string' ? header.parentSession : null
        if (parent) root = sessionRoots.get(parent) || parent
      } catch (err) {}
      sessionRoots.set(sid, root)
      return root
    }

    function startTurn(rootId, turn) {
      const prev = turnAggs.get(rootId)
      if (prev && prev.turn !== turn) finalizeTurn(rootId)
      turnAggs.set(rootId, { turn, cost: 0, tokens: 0, lastTs: Date.now() })
    }

    function finalizeTurn(rootId) {
      const agg = turnAggs.get(rootId)
      if (agg && (agg.tokens > 0 || agg.cost > 0)) {
        lastTurn = { turn: agg.turn, amount: agg.cost, tokens: agg.tokens, ts: agg.lastTs }
        lastTurnSeq++
      }
      turnAggs.delete(rootId)
    }
    // 监听会话事件流：usage 只在 assistant/message 携带；
    // 根会话的 turn/start 开桶、turn/end 结算本轮；子代理 usage 并入根桶。
    // token 采用 DSH 互斥分桶口径：input + cacheRead + cacheWrite + output
    // （推理 token 已包含在 outputTokens 中，单加 reasoningTokens 会重复计）。
    function handleSessionEvent(session, event) {
      try {
        ...
        if (type === 'turn/start') {
          if (sid === root) startTurn(root, Number(d.turn))
          return
        }
        if (type === 'turn/end') {
          if (sid === root) finalizeTurn(root)
          return
        }
        ...
        let agg = turnAggs.get(root)
        if (!agg && sid === root) {
          // 插件中途加载等场景错过 turn/start：根会话按消息自带 turn 开桶
          const turn = Number(d.turn)
          if (isFinite(turn)) {
            agg = { turn, cost: 0, tokens: 0, lastTs: Date.now() }
            turnAggs.set(root, agg)
          }
        }
        if (agg) {
          agg.tokens += tokens
          // 定价换算（CNY/百万 token；缓存命中=hit 档；缓存写入按未命中输入价计）
          const model = d.message && d.message.source ? d.message.source.model : ''
          const p = priceFor(model)
          const off = isPeakTime(Math.floor(Date.now() / 1000)) ? 1 : 0
          agg.cost += (cacheRead / 1e6) * p.hit[off] + ((input + cacheWrite) / 1e6) * p.miss[off] + (output / 1e6) * p.out[off]
          agg.lastTs = Date.now()
        }
        // 对话累计（含子代理，跨轮不清零）
        let st = sessionTotals.get(root)
        if (!st) { st = { tokens: 0, calls: 0, lastTs: 0 }; sessionTotals.set(root, st) }
        st.tokens += tokens
        st.calls += 1
        st.lastTs = Date.now()
        currentRootId = root
    ...
    disposers.push(ctx.on('session/disposed', (session) => {
      ...
      const root = sessionRoots.get(sid) || sid
      sessionRoots.delete(sid)
      if (root === sid) turnAggs.delete(sid)
    }))

    // ── 权威数据源：DSH 会话投影服务 ──
    // tokenUsage 投影从会话「完整日志」折叠（含重启前恢复的历史），与 DSH
    // 统计条同源同口径；事件累加只在投影不可用时兜底（进程内有效，重启清零）。
    let projectionsSvc = null
    let sessionsSvc = null
    try {
      ctx.inject(['sessionProjections', 'sessions'], (scope) => {
        projectionsSvc = (scope && scope.sessionProjections) || null
        sessionsSvc = (scope && scope.sessions) || null
      })
    } catch (err) {}

    const TOKEN_USAGE_KEY = 'tokenUsage'
    let projCache = { at: 0, stats: null }

    function projectionTokensOf(session) {
      try {
        if (!projectionsSvc || !session) return null
        const state = projectionsSvc.stateOf(session, TOKEN_USAGE_KEY)
    ...
    // 存储兜底：进程刚启动、尚无任何事件时，用磁盘上最近写入的会话文件
    // 定位「当前对话」（含子代理文件——沿 parentSession 上溯即得其根会话）
    function newestStoredSessionId() {
      try {
        const rootDir = path.join(DSH_HOME, 'sessions')
        let best = null
        for (const ws of fs.readdirSync(rootDir)) {
          const wsDir = path.join(rootDir, ws)
          let entries
          try { entries = fs.readdirSync(wsDir) } catch (err) { continue }
          for (const name of entries) {
            if (name.indexOf('session-') !== 0) continue
            const file = path.join(wsDir, name, 'session.jsonl.zstd')
            try {
              const st = fs.statSync(file)
              if (!best || st.mtimeMs > best.mtimeMs) best = { mtimeMs: st.mtimeMs, id: name }
            } catch (err) {}
          }
        }
    ...
    function sessionInTreeOf(session, root) {
      let cur = session
      let depth = 0
      while (cur && depth < 16) {
        if (cur === root || (cur.id && cur.id === root.id)) return true
        const parent = cur.header && typeof cur.header.parentSession === 'string' ? cur.header.parentSession : null
        if (!parent) return false
        cur = sessionsSvc.get(parent)
        depth++
      }
      return false
    }

    // 当前对话的权威统计：根会话投影 + 内存中全部派生会话（子代理）投影合计
    function getProjectionStats() {
      const now = Date.now()
      if (projCache.stats && now - projCache.at < 900) return projCache.stats
      let stats = null
      try {
        let root = null
        if (currentRootId && sessionsSvc) root = sessionsSvc.get(currentRootId) || null
        if (!root && sessionsSvc) {
          const newest = newestStoredSessionId()
          if (newest) {
            let cur = sessionsSvc.get(newest) || null
            let depth = 0
            while (cur && depth < 16) {
              const parent = cur.header && typeof cur.header.parentSession === 'string' ? cur.header.parentSession : null
              if (!parent) break
              const next = sessionsSvc.get(parent)
              if (!next) break
              cur = next
              depth++
            }
        ...
        if (root && sessionsSvc) {
          let tokens = 0, treeSessions = 0
          for (const s of sessionsSvc.list()) {
            if (!s || !sessionInTreeOf(s, root)) continue
            const t = projectionTokensOf(s)
            if (t !== null) { tokens += t; treeSessions++ }
          }
          if (treeSessions > 0) stats = { sessionId: root.id, tokens, sessions: treeSessions }
        }
      } catch (err) {}
      projCache = { at: now, stats }
      return stats
    }

    function getSessionStats() {
      // 优先权威投影（覆盖重启前历史，与 DSH 统计条一致）
      const proj = getProjectionStats()
      if (proj) {
        const ev = sessionTotals.get(proj.sessionId)
        return { sessionId: proj.sessionId, tokens: proj.tokens, calls: ev ? ev.calls : null }
      }
      // 兜底：进程内事件累加（重启后近似为零）
      if (currentRootId && sessionTotals.has(currentRootId)) {
        const st = sessionTotals.get(currentRootId)
        return { sessionId: currentRootId, tokens: st.tokens, calls: st.calls }
      }
      // 没有活动事件时，取最近活跃的对话近似"当前"
      let best = null
      for (const [sid, st] of sessionTotals) {
        if (!best || st.lastTs > best.lastTs) best = { sid, st }
      }
      return best ? { sessionId: best.sid, tokens: best.st.tokens, calls: best.st.calls } : { sessionId: null, tokens: 0, calls: 0 }
    }

    // 供 balance.json 使用：大数字=当前对话 token（含子代理），小字=今日 token（读 DSH 账本）
    function getUsageStats() {
      const s = getSessionStats()
      const t = readTodayTokens()
      return {
        ok: true,
        totalBalance: s.tokens,
        currency: 'tokens',
        provider: '当前对话',
        balanceKind: 'session',
        calls: s.calls,
        todayUsage: t ? t.tokens : null,
        todayCalls: t ? t.calls : null,
    ...
    // ===== 本地修改：从「DeepSeek 官方余额」改为「当前提供商余额；查不到则显示今日 token 消耗」=====
    function fetchJSON(url, key) {
      return fetch(url, { headers: { Authorization: 'Bearer ' + key }, signal: AbortSignal.timeout(15000) })
        .then((res) => (res.ok ? res.json().catch(() => null) : Promise.reject(new Error('HTTP ' + res.status))))
    }

    // 本地日期键：账本按本地日写入，用 UTC 的 toISOString 会把
    // 东八区 0:00–8:00 的「今天」读成「昨天」
    ...
    // 读 DSH 自己的用量账本（~/.dsh/dsh-usage/usage-ledger.json），取今天的 token 消耗
    function readTodayTokens() {
      try {
        const ledgerPath = path.join(DSH_HOME, 'dsh-usage', 'usage-ledger.json')
        const ledger = JSON.parse(fs.readFileSync(ledgerPath, 'utf8'))
        const dayKey = localDayKey(Date.now())
        const day = ledger && ledger.days && ledger.days[dayKey]
        if (!day || typeof day !== 'object') return null
        let tokens = 0, calls = 0
        for (const pv of Object.values(day)) {
          if (!pv || typeof pv !== 'object') continue
          for (const m of Object.values(pv)) {
            if (!m || typeof m !== 'object') continue
            // 互斥分桶全量：输入 + 缓存读 + 缓存写 + 输出（缓存往往占大头，漏掉会严重偏低）
            tokens += (Number(m.inputTokens) || 0) + (Number(m.outputTokens) || 0) + (Number(m.cacheReadTokens) || 0) + (Number(m.cacheWriteTokens) || 0)
    ...
    // hybgzs（当前模型 glm-5.3-flash 的提供商）：OpenAI 兼容计费接口
    // 剩余余额 = 额度上限（hard/soft_limit_usd）- 已用（total_usage，单位美分）
    async function fetchHybgzsBalance() {
      let cred
      try {
        cred = await ctx.credentials.resolve('HYBGZS_API_KEY')
      } catch (err) {
        return { ok: false, code: 'NO_KEY', error: '凭据读取失败: ' + String((err && err.message) || err).slice(0, 160) }
      }
      if (!cred) return { ok: false, code: 'NO_KEY', error: '未配置 HYBGZS_API_KEY' }
      const base = 'https://ai.hybgzs.com/v1'
      const today = new Date().toISOString().slice(0, 10)
      try {
        const [sub, use] = await Promise.all([
          fetchJSON(base + '/dashboard/billing/subscription', cred.value),
          fetchJSON(base + '/dashboard/billing/usage?start_date=2026-01-01&end_date=' + today, cred.value),
        ])
        const limit = Number(sub && (sub.hard_limit_usd ?? sub.soft_limit_usd ?? sub.system_hard_limit_usd))
        const usedCents = Number(use && use.total_usage)
        if (!isFinite(limit) || !isFinite(usedCents)) {
          return { ok: false, code: 'SHAPE', error: 'hybgzs 计费接口返回结构异常' }
        }
        return {
          ok: true,
          totalBalance: limit - usedCents / 100,
          currency: 'USD',
          provider: 'hybgzs',
          balanceKind: 'balance',
        ...
        return { ok: false, code: 'HTTP', transient: true, error: 'hybgzs 余额接口失败: ' + String((err && err.message) || err).slice(0, 200) }
      }
    }

    async function fetchBalance() {
      // 1) 先查当前提供商余额
      const bal = await fetchHybgzsBalance()
      if (bal.ok) return bal
      // 2) 查不到余额：降级显示今日 token 消耗
      const t = readTodayTokens()
      if (t && t.tokens > 0) {
        return {
          ok: true,
          totalBalance: t.tokens,
          currency: 'tokens',
          provider: 'hybgzs',
          balanceKind: 'tokens',
          calls: t.calls,
          note: bal.error || '提供商无余额接口，改显示今日消耗 token 量',
          updatedAt: new Date().toISOString(),
        }
      }
      return bal
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        const payload = lastTurn
          ? { ok: true, seq: lastTurnSeq, turn: lastTurn.turn, amount: lastTurn.amount, tokens: lastTurn.tokens, ts: lastTurn.ts, sessionTokens: s.tokens, todayTokens: t ? t.tokens : null }
          : { ok: true, seq: 0, turn: null, amount: null, tokens: null, ts: null, sessionTokens: s.tokens, todayTokens: t ? t.tokens : null }
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify(payload))
      },
    }))
