...
function apply(ctx) {
    ...
    // 当前会话累计 token（含子代理事件）：assistant/message 每步累加，跨轮不清零
    let sessionTotals = new Map() // sessionId -> { tokens, calls, lastTs }
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        agg.lastTs = Date.now()
        // 会话累计：同一步 usage 同时累加进会话总账（跨轮不清零）
        let st = sessionTotals.get(sessionId)
        if (!st) { st = { tokens: 0, calls: 0, lastTs: 0 }; sessionTotals.set(sessionId, st) }
        st.tokens += input + cache + output + reasoning
        st.calls += 1
        st.lastTs = Date.now()
    ...
    function getSessionStats() {
      if (currentSessionId && sessionTotals.has(currentSessionId)) {
        const st = sessionTotals.get(currentSessionId)
        return { sessionId: currentSessionId, tokens: st.tokens, calls: st.calls }
      }
      // 没有活动事件时，取最近活跃的会话近似"当前"
      let best = null
    ...
    // 供 balance.json 使用：大数字=当前会话 token，小字=今日 token（读 DSH 账本）
